...
def main(args):
    ...
    # prepare dataset
    if os.path.exists("multilingual_split.npy"):
        dali_split = np.load('multilingual_split.npy', allow_pickle=True).item()
    else:
        print("Full split not found, try loading dummy split for DEBUG")
        dali_split = np.load('dummy_split.npy', allow_pickle=True).item()

    print("Initializing datasets...")
    val_data = LyricsAlignDataset(dali_split, "val", args.sr, hparams['input_sample'], args.sepa_dir)
    train_data = LyricsAlignDataset(dali_split, "train", args.sr, hparams['input_sample'], args.sepa_dir)
    ...
    if args.model == "baseline":
        criterion = nn.CTCLoss(blank=71, zero_infinity=True)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sepa_dir', type=str, default='/import/c4dm-datasets/sepa_DALI/audio_umx_16000/',
                        help="Folder of source-separated vocals; Filename is '{ID}_vocals.wav'")
